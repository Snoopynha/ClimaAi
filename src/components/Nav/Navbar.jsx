...
import { FaSearch, FaSun, FaMoon, FaCog, FaBars } from 'react-icons/fa';
    ...
    const delayDebounce = setTimeout(async () => {
      try {
        const resp = await fetch(
          `https://geocoding-api.open-meteo.com/v1/search?name=${encodeURIComponent(cidade)}&count=5`
        );
  ...
  return (
    <div className="nav">
      <nav className={`navbar ${modoEscuro ? 'dark' : ''}`}>
        ...
        <div className={`direita ${menuAberto ? 'menu-aberto' : ''}`}>
          <button className="btn-menu" onClick={() => setMenuAberto(!menuAberto)} aria-label="Menu">
            <FaBars />
          </button>
          ...
          <div className="configuracoes">
            <button onClick={() => setMostrarOpcoes(!mostrarOpcoes)} aria-haspopup="true" aria-expanded={mostrarOpcoes}>
              <FaCog />
            </button>
            {mostrarOpcoes && (
          ...
          <button className="modo-escuro-btn" id="btn" onClick={toggleModoEscuro} aria-pressed={modoEscuro}>
